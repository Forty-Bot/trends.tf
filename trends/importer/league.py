...
def import_team(c, t):
    ...
    c.execute(
        f"""INSERT INTO league_team (league, teamid, team_nameid, avatarhash, fetched)
            VALUES (
                %(league)s, {teamid_col},
                (SELECT team_nameid FROM team_name WHERE team = %(name)s), %(avatarhash)s,
    ...
    c.execute(
        """INSERT INTO team_comp (
               league, teamid, compid, team_nameid, rgl_teamid, end_rank, fetched
           ) VALUES (
               %(league)s, %(teamid)s, %(compid)s,
               (SELECT team_nameid FROM team_name WHERE team = %(name)s), %(rgl_teamid)s,
               %(end_rank)s, CASE WHEN league_team_per_comp(%(league)s) THEN %(fetched)s END
           ) ON CONFLICT (league, teamid, compid)
           DO UPDATE SET
                team_nameid = EXCLUDED.team_nameid,
                end_rank = EXCLUDED.end_rank,
                fetched = greatest(EXCLUDED.fetched, team_comp.fetched);""", t)
...
def import_transfers(c, t):
    ...
    psycopg2.extras.execute_values(c, "INSERT INTO old_ranges (steamid64, rostered) VALUES %s;",
    ...
    c.execute(
        """DELETE FROM team_player
           ...
           WHERE league = %(league)s
               ...
               AND team_player.steamid64 = new_ranges.steamid64
               AND team_player.rostered = new_ranges.old
               AND new_ranges.old != new_ranges.new;""", t)
    c.execute(
        """INSERT INTO team_player (league, teamid, compid, steamid64, rostered)
           SELECT
               %(league)s,
               %(teamid)s,
...
def import_match(c, m):
    ...
    c.execute(
        """INSERT INTO match (
               league, matchid, compid, divid, teamid1, teamid2, round_seq, scheduled, mapids,
               score1, score2, forfeit, fetched
           ) VALUES (
